...
public class Selector {
	...
	public void selectTo(int r1, int r2) {
		...
		while (i.hasNext()) {
			int p = i.next().intValue();
			if (p == r1) { cpu.getRegister(r1).setValue(outBus.getValue()); }
			if (p == r2) { cpu.getRegister(r2).setValue(outBus.getValue()); }
		}
	}
	...
	public void selectTo(int r1, int r2, int r3) {
		...
		while (i.hasNext()) {
			...
			if (p == r1) { cpu.getRegister(r1).setValue(outBus.getValue()); }
			if (p == r2) { cpu.getRegister(r2).setValue(outBus.getValue()); }
			if (p == r3) { cpu.getRegister(r3).setValue(outBus.getValue()); }
		}
	}
}
