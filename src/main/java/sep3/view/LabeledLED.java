...
public class LabeledLED extends JPanel {
	...
	public LabeledLED(int color, String label, String pos) {
		this(color, label, 10, pos);
	}    // デフォルトのラベルフォントの大きさptは10

	public LabeledLED(int color, String label) {
		this(color, label, 10, "South");
	}                // デフォルトの位置は、ボタンの下

	public LabeledLED(int color, String label, int pt) {
		this(color, label, pt, "South");
	}        // ptだけ変える

	public LabeledLED(int color) {
		this(color, "");
	}                                            // 何も書いてないときは、ラベル文字列なし
