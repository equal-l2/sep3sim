...
public class LabeledLED7segment extends JPanel {
	...
	public LabeledLED7segment(String label, int labelPt, String pos) {
		this(label, labelPt, pos, 20);
	}    // LEDの表示はデフォルト20pt

	public LabeledLED7segment(String label, String pos) {
		this(label, 10, pos);
	}    // デフォルトのラベルフォントの大きさptは10

	public LabeledLED7segment(String label) {
		this(label, 10, "South");
	}            // デフォルトの位置は、ボタンの下

	public LabeledLED7segment(String label, int pt) {
		this(label, pt, "South");
	}    // ptだけ変える

	public LabeledLED7segment() {
		this("");
	}                                        // 何も書いてないときは、ラベル文字列なし

	public LED7segment getLED7segment() {
		return led7;
	}
}
