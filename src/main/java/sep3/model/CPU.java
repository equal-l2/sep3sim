...
public class CPU {
	...
	private final InstructionSet iset;                // SEP-3 の命令集合
	private final Bus aBus;
	private final Bus bBus;
	private final Bus sBus;    // A, B, S の各内部バス
	...
	public Selector getABusSelector() {
		return aBus.getSelector();
	}

	public Selector getBBusSelector() {
		return bBus.getSelector();
	}

	public Selector getSBusSelector() {
		return sBus.getSelector();
	}
	...
	public OnOffFlag getHaltLamp() {
		return halted;
	}

	public OnOffFlag getIllLamp() {
		return illegal;
	}
	...
	// CPUリセット
	public void reset() {
		for (int i = REG_R0; i <= REG_R5; ++i) {
			regs[i].setInitValue(0);
		}
